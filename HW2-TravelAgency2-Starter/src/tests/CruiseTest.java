...
public class CruiseTest 
{
	...
	public void testConstrutor()
	{
		...
		assertEquals(99.75, c.getDepositAmount(), DOUBLE_TOLERANCE);
	}
	
	@Test
	public void testAddExcrusionValid()
	{
		date1.set(2019, 4, 1, 10, 0);
		date2.set(2019, 4, 4, 14, 0);
		Cruise c = new Cruise("Floating Germ Palace", 4, "Knight", "Miami, FL", date1, date2, 199.5);
		...
		assertEquals("Water slide", c.getExcursions()[0]);
		assertEquals(200.0, c.getPrice(), DOUBLE_TOLERANCE);
	}
	...
	public void testAddExcursionEmpty()
	{
		...
		c.addExcursion(null, 0.5);
		assertEquals(0, c.getNumExcursions());
		assertEquals(200.5, c.getPrice(), DOUBLE_TOLERANCE);
	}
	
	@Test
	public void testAddExcursionNegative()
	{
		date1.set(2019, 4, 1, 10, 0);
		date2.set(2019, 4, 4, 14, 0);
		Cruise c = new Cruise("Floating Germ Palace", 4, "Knight", "Miami, FL", date1, date2, 199.5);
	...
	public void testLodging()
	{
		...
		assertEquals(200 * 1.5, c.getLodgingCost(), DOUBLE_TOLERANCE);
		c.setCabinType(CabinType.INTERIOR);
		assertEquals(CabinType.INTERIOR, c.getCabinType());
		assertEquals(200, c.getLodgingCost(), DOUBLE_TOLERANCE);
		c.setCabinType(CabinType.SUITE);
		assertEquals(CabinType.SUITE, c.getCabinType());
		assertEquals(200 * 5, c.getLodgingCost(), DOUBLE_TOLERANCE);
	}
	...
	public void testToString()
	{
		...
		c.addExcursion("Water slide", -0.5);
		assertEquals("$  200.00  Hi (Flight Not Included)\n           Cruising from Miami on the Knight (includes 1excursions)", c.toString());
	}
}
