...
public class VacationPackageCollection
{
	 ...
	 */
	private static final int MAX_PKGS = 25;
	/**
	 ...
	 */
	private VacationPackage[] collection = new VacationPackage[MAX_PKGS];
	 ...
	 * @param trip A VacationPackage object to be added to the Collection.
	 */
	public void addVacation(VacationPackage trip)
	{
		if (this.numTrips < MAX_PKGS)
		{
	 ...
	 * cannot be added in the first place, and packages which potentially could (but do not yet)
	 * have flight data.
	 * 
	...
	private void lexiSort()
	{
		...
		{
			smallIndex = i;
			smallVal = this.collection[i].getName();
			for (int j = i + 1; j < this.numTrips; j++)
			{
				if (this.collection[j].getName().compareTo(smallVal) < 0)
				{
					smallIndex = j;
					smallVal = this.collection[j].getName();
				}
			}
			this.swapArray(i, smallIndex);
		}
